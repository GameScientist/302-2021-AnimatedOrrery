...
public class CameraControls : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        dollyDis = Slide(dollyDis, targetDollyDis, .05f);

        cam.transform.localPosition = subject.position + new Vector3(0, dollyDis, -dollyDis);
        ...
        pitch = Slide(pitch, targetPitch, .05f);
        yaw = Slide(yaw, targetYaw, .05f);

        transform.rotation = Quaternion.Euler(pitch, yaw, 0);
    }

    private static float Slide(float current, float target, float percentLeftAfter1Second) // The camera eases into new positions.
    {
